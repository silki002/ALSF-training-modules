...
stats_df <- mutate(
  stats_df, # data frame we'd like to add a variable to
  neg_log10_p = -log10(p_value) # column name and values
)

names(stats_df)
stats_df$neg_log10_p[1:5]
```
...
ggplot(
  ...
  geom_point(alpha = 0.2) +
  geom_hline(yintercept = 5.5, color = "firebrick4") +
  # Add on this set of appearance presets to make it pretty
  theme_bw() 
```
...
male_female_df%>%
  ...
  geom_point(alpha = 0.2) +
  geom_hline(yintercept = 2.5, color = "#1BAEC6") +
  theme_bw() +
  labs(
    x = "log2 Fold Change Tumor/Normal",
    y = "-log10 p value",
  ...
  scale_color_gradient(low = "#4a2454", high = "#96689A")
```
