...
file.exists("gene_results_GSE44971.tsv")
```
...
stats_df%>%
  filter(contrast == "male_female", avg_expression > 50)
```
...
stats_df%>%
  ...
  select(gene_symbol, log_fold_change, t_statistic)
```
...
stats_df |>
  ...
  select(neg_log10_p_value, everything())
```
...
stats_df |>
  summarize(
    mean_lf = mean(log_fold_change),
    sd_lf = sd(log_fold_change)
    )
```
...
readr::write_tsv(stats_summary_df, file.path("results", "stats_summary.tsv"))
```
...
readr::write_rds(stats_summary_df, file.path("results", "stats_summary.RDS"))
```
...
reimport_df <- read_rds(file.path("results", "stats_summary.RDS"))
```
