...
# Turns into a vector of characters
```
...
metadata_file <- file.path("data", "metadata_GSE44971.tsv")
```
...
class(metadata$data_row_count) # numeric
class(metadata$refinebio_accession_code) # character
...
brain_loc <- metadata$`brain location` # vector
```
...
brain_loc <- as.factor(brain_loc)
summary(brain_loc) # Diencephalon
```
...
cerebral <- metadata$`brain location` == "Cerebral hemisphere"
```
